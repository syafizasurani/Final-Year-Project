...
if(isset($_POST['scalingdata'])){
    $id = $_POST['scaling_id'];

    $sqlinsertappt = "INSERT INTO scaling_history SELECT * FROM scaling_appointment WHERE id=$id";
    $stmtins = $conn->prepare($sqlinsertappt);
    $stmtins->execute();

    $sqldeleteappt = "DELETE FROM scaling_appointment WHERE id=$id";
