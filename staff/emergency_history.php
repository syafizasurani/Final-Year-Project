...
<body>
    ...
    <div class="w3-container w3-white" style="padding:150px 200px">
        <div class="w3-center w3-container" style="padding-top:10px; padding-bottom:10px">
            <h4><b>Emergency Case History</b></h4>
        </div>
        <div class="w3-display w3-center w3-padding w3-container">
            <form class="search w3-padding-16" action="emergency_history.php" method="POST">
                <input type="text" name="valueToSearchEmergency" placeholder="&nbsp Search by matric no." style="border-radius:10px; width:60%; height:38px">
                <button type="submit" name="search_emergency" value="Filter" style="border-radius:10px; background-color:#3E42A3; color:white; height:38px; width:15%"><i class="fa fa-search" style="color:white"></i>&nbsp Search</button>
            </form>
        </div>
        <div class="w3-display w3-center w3-padding w3-container">
            <table class="w3-table w3-striped w3-bordered w3-border w3-hoverable w3-white" id="table">
                ...
                <tbody>
                    <?php 
                        if(isset($_POST['search_emergency'])){
                            $valueToSearchEmergency = $_POST['valueToSearchEmergency'];
                            $query = "SELECT * FROM emergency_case WHERE `matric_no` LIKE  '%".$valueToSearchEmergency."%'";
                            $search_results_emergency = filterTable_emergency($query);
                        }
                        else{
                            $query = "SELECT * FROM emergency_case";
                            $search_results_emergency = filterTable_emergency($query);
                        }
                        function filterTable_emergency($query){
                            $connect = mysqli_connect('localhost', 'root', "", "hep_staff");
                            $filter_Results_emergency = mysqli_query($connect, $query);
                            return $filter_Results_emergency;
                        }
                    ?>
                    <?php while($result = mysqli_fetch_array($search_results_emergency)): ?>    
                        <tr>
                            ...
                            <td style="text-align:center"> <?php echo $result['case_date']; ?> </td>
                            <td style="text-align:center"> <?php echo $result['case_time']; ?> </td>
                            <td style="text-align:center"> <?php echo $result['phone_no']; ?> </td>
                            <td style="text-align:center"> <?php echo $result['case_details']; ?> </td>
                        </tr>                            
                    <?php endwhile; ?>      
                </tbody>
            </table>
